...
func TestWordLine(t *testing.T) {
	Convey("Word Lines with...", t, func() {
		...
		Convey("Empty Strings", func() {
			wl := NewWordLine("", paint.DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
		...
		Convey("One Word", func() {
			wl := NewWordLine("word", paint.DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
			So(wl.Value(), ShouldEqual, "word")
			So(wl.Words(), ShouldHaveLength, 1)
			So(wl.CharacterCount(), ShouldEqual, 4)
			So(wl.WordCount(), ShouldEqual, 1)
			err := wl.AppendWordRune(0, '!', paint.DefaultMonoStyle)
			So(err, ShouldBeNil)
			So(wl.Value(), ShouldEqual, "word!")
			err = wl.AppendWordRune(1, '#', paint.DefaultMonoStyle)
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldEqual, "word at index 1 not found")
		})
		Convey("More Than One Word", func() {
			wl := NewWordLine("more than words", paint.DefaultMonoStyle)
			So(wl, ShouldNotBeNil)
		...
		Convey("Making things...", func() {
			Convey("wrapping on...", func() {
				Convey("word", func() {
					wl := NewWordLine("one two three", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, enums.WRAP_WORD, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Len(), ShouldEqual, 4)
					So(lines[1].Len(), ShouldEqual, 1)
					wl = NewWordLine("one two three\nfour five six", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_WORD, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					...
					wl = NewWordLine("1234567890ABCDEF", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_WORD, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					...
					wl = NewWordLine("1234567890   DEF", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_WORD, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "1234567890")
					...
					So(lines[1].Len(), ShouldEqual, 1)
					wl = NewWordLine("1234567890   ABC DEF", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_WORD, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "1234567890")
				...
				Convey("char", func() {
					wl := NewWordLine("one two three", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, enums.WRAP_CHAR, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					...
					So(lines[1].CharacterCount(), ShouldEqual, 7)
					So(lines[1].Len(), ShouldEqual, 3)
					wl = NewWordLine("1234567890   ABC\nDEF word", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_CHAR, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					...
					So(lines[2].Len(), ShouldEqual, 3)
					wl = NewWordLine("1234567890   ABC\nDEF word\nmoar words!", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_CHAR, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
				...
				Convey("word-char", func() {
					wl := NewWordLine("one two three", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, enums.WRAP_WORD_CHAR, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "one two ")
				...
				Convey("none", func() {
					wl := NewWordLine("one two three", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines := wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 1)
					So(lines[0].Value(), ShouldEqual, "one two th")
					So(lines[0].CharacterCount(), ShouldEqual, 10)
					So(lines[0].Len(), ShouldEqual, 5)
					...
					So(lines[0].CharacterCount(), ShouldEqual, 10)
					So(lines[0].Len(), ShouldEqual, 1)
					wl = NewWordLine("one two three\n1234567890ABCDEF", paint.DefaultMonoStyle)
					So(wl, ShouldNotBeNil)
					lines = wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
					So(lines, ShouldHaveLength, 2)
					So(lines[0].Value(), ShouldEqual, "one two th")
			...
			Convey("aligning to...", func() {
				wl := NewWordLine("one", paint.DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines := wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_LEFT, 10, paint.DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				...
				So(lines[0].Len(), ShouldEqual, 1)
				lines = wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_RIGHT, 10, paint.DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "       one")
				So(lines[0].CharacterCount(), ShouldEqual, 10)
				So(lines[0].Len(), ShouldEqual, 8)
				lines = wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_CENTER, 10, paint.DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "    one")
				So(lines[0].CharacterCount(), ShouldEqual, 7)
				So(lines[0].Len(), ShouldEqual, 5)
				wl = NewWordLine("one two", paint.DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines = wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_FILL, 10, paint.DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
				So(lines[0].Value(), ShouldEqual, "one    two")
				So(lines[0].CharacterCount(), ShouldEqual, 10)
				So(lines[0].Len(), ShouldEqual, 3)
				wl = NewWordLine("1 2 3 4", paint.DefaultMonoStyle)
				So(wl, ShouldNotBeNil)
				lines = wl.Make(false, enums.WRAP_NONE, false, enums.JUSTIFY_FILL, 10, paint.DefaultMonoStyle)
				So(lines, ShouldHaveLength, 1)
