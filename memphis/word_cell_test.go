...
func TestWordCell(t *testing.T) {
	Convey("Word Cells with...", t, func() {
		Convey("Empty Strings", func() {
			wc := NewWordCell("", paint.DefaultMonoStyle)
		...
		Convey("One Word", func() {
			wc := NewWordCell("word", paint.DefaultMonoStyle)
			So(wc, ShouldNotBeNil)
		...
		Convey("More Than One Word", func() {
			wc := NewWordCell("another word", paint.DefaultMonoStyle)
			So(wc, ShouldNotBeNil)
		...
		Convey("Basic checks", func() {
			...
			wc.AppendRune(' ', paint.DefaultMonoStyle)
			So(wc.Len(), ShouldEqual, 1)
			c := wc.GetCharacter(0)
			So(c, ShouldNotBeNil)
			So(c.Value(), ShouldEqual, ' ')
			So(c.Style().String(), ShouldEqual, paint.DefaultMonoStyle.String())
